        ...
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form class="space-y-6" @submit.prevent="handleForm">
                ...
                <FormGroup v-if="props.formType == 'signup'" label="Full name" name="name" type="text"
                    autocomplete="off" required labelFor="name" v-model="userForm.name" :errorMessage="errorBag.name" />
            ...
            <p class="mt-10 text-center text-sm text-gray-500">
                <span v-if="props.formType != 'signup'">
                    <NuxtLink to="/signup" class="font-semibold leading-6 text-indigo-600 hover:text-indigo-500">Sign Up
                    </NuxtLink>
                </span>
